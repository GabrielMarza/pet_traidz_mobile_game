...
var zim = function(zim) {
    ...
    zim.LeaderBoard = function(data, title, width, height, corner, backgroundColor, titleColor, colors, total, scoreWidth, scorePlaces, scoreZeros, spacing, arrows, borderColor, borderWidth, shadowColor, shadowBlur, reverse, allowZero, font, fontSize, nameShift, scoreShift) {
        ...
        var c = {
            rankBackgroundColor: frame.darker,
            rankColor: frame.light,
            currentRankBackgroundColor: frame.pink,
            currentRankColor: "white",

            nameBackgroundColor: frame.lighter,
            nameColor: frame.darker,
            currentNameBackgroundColor: "#f0d2e8",
            currentNameColor: frame.darker,

            scoreBackgroundColor: frame.light,
            scoreColor: frame.darker,
            currentScoreBackgroundColor: "#f0d2e8",
            currentScoreColor: frame.darker
        }
            ...
            for (var j=0; j<total; j++) {
                ...
                for (var i=0; i<3; i++) {
                    stepper = new zim.Stepper({
            			...
            			width:100,
            			continuous:true,
                        label:new zim.Label({text:"", font:font, color:j==that.winner?c.currentNameColor:c.nameColor, size:fontSize+4, align:"center"}),
                    ...
                    scoreText.x = rank.width + spacing + 3*(stepper.width+spacing);
                    if (arrows && !zot(that.winner)) {
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    zim.Board = function(size, num, backgroundColor, rollBackgroundColor, borderColor, borderWidth, icon, isometric, indicatorColor, indicatorBorderColor, indicatorBorderWidth, indicatorSize, indicatorType, arrows, arrowColor, arrowRollColor, swipe, info, labels, color, scaleMin, scaleMax, swipe, buffer) {
        ...
        if (zot(info)) {
            ...
            var i0 = 0;
            // get most cols
            zim.loop(Math.max(num,info.length), function (j) {
                if (info[j] && info[j].length > i0) i0 = info[j].length;
            });
            zim.loop(Math.max(num,info.length), function (j) {
                var inf = info2[j] = [];
                zim.loop(Math.max(num, i0), function (i) {
        ...
        tiles = this.tiles = new zim.Tile(container.centerReg(), num, num);
        if (isometric) {
            tiles.rot(45)
                .centerReg(holder)
            holder.sca(2,1);
        ...
        Object.defineProperty(this, 'startCol', {
            ...
            set: function(value) {
                startCol = zim.constrain(value, 0, that.numCols-num);
        ...
        Object.defineProperty(this, 'startRow', {
            ...
            set: function(value) {
                startRow = zim.constrain(value, 0, that.numRows-num);
        ...
        Object.defineProperty(this, 'data', {
            ...
            get: function() {
                var data = [];
                zim.loop(startRow+num, function (j) {
                    var d = [];
                    data.push(d);
                    zim.loop(startCol+num, function (i) {
            ...
            set: function(value) {
                if (value != isometric) {
                    ...
                    if (swipe) {
                        ...
                        if (that.swipeEvent) that.swipeEvent.off("swipe", that.swipeEvent);
        ...
        this.update = function() {
            ...
            zim.loop(num+startRow, function (j) {
                zim.loop(num+startCol, function (i) {
        ...
        this.moveCamera = function(dir) {
            ...
            if (dir == "left") {
                if (that.arrowRight && startCol == that.numCols-num) that.arrowRight.hov(arrowRollColor);
                ...
                that.startCol = startCol+1;
                if (that.arrowRight && startCol == that.numCols-num) that.arrowRight.hov(-1);
            } else if (dir == "up") {
                if (that.arrowDown && startRow == that.numRows-num) that.arrowDown.hov(arrowRollColor);
                ...
                that.startRow = startRow+1;
                if (that.arrowDown && startRow == that.numRows-num) that.arrowDown.hov(-1);
            }
            return that;
        }
        ...
        this.setArrowHover = function() {
            ...
            if (that.arrowRight) that.arrowRight.hov(startCol==that.numCols-num?-1:arrowRollColor);
            if (that.arrowUp) that.arrowUp.hov(startRow==0?-1:arrowRollColor);
            if (that.arrowDown) that.arrowDown.hov(startRow==that.numRows-num?-1:arrowRollColor);
        }
        ...
        this.getTile = function(col, row) {
            ...
            if (col<0||row<0||col>that.num-1||row>that.num-1) return;
            return tiles.getChildAt(row*num+col);
        }
        ...
        this.getRandomTile = function(filter) {
            ...
            while(!tile && count<10000) {
                var i = zim.rand(0,num-1)+startCol;
                var j = zim.rand(0,num-1)+startRow;
        ...
        this.position = function(obj, col, row, internal) {
            ...
            if (col < 0 || col > num-1 || row < 0 || row > num-1) obj.visible = false;
            else {
            ...
            if (tile) {
                if (isometric) obj.sca(obj.proportion.convert(obj.y));
                that.setDepth();
            }
        ...
        this.add = function(obj, col, row, data, color, icon, internal) {
            ...
            if (isometric && !obj.boardEvent) {
                obj.boardEvent = obj.on("animation", function () {
                    if (obj.visible) obj.sca(obj.proportion.convert(obj.y));
        ...
        this.move = function(obj, col, row, time, type) {
            ...
            obj.visible = true;
            var newCol = zim.constrain(obj.boardCol+col, 0, num-1);
            var newRow = zim.constrain(obj.boardRow+row, 0, num-1);
            ...
            if (obj.boardCol!=newCol || obj.boardRow!=newRow) obj.animate({
                ...
                call:function () {
                    if (that.buffer && that.buffer > 0) {
                        ...
                        var right = (obj.boardCol < newCol && num-obj.boardCol-1 <= that.buffer);
                        var up = (obj.boardRow > newRow && obj.boardRow <= that.buffer);
                        var down = (obj.boardRow < newRow && num-obj.boardRow-1 <= that.buffer);
                    }
                    ...
                    obj.animate({
                        ...
                        call:function () {
                            if (tile && buffer && buffer > 0) {
                                ...
                                var right = (obj.boardCol < newCol && num-obj.boardCol-1 <= buffer);
                                var up = (obj.boardRow > newRow && obj.boardRow <= buffer);
                                var down = (obj.boardRow < newRow && num-obj.boardRow-1 <= buffer);
                            }
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    zim.Timer = function(time, step, colon, down, isometric, startPaused, size, font, color, backgroundColor, borderColor, borderWidth, align, valign, fontOptions, width, height) {
        ...
        this.super_constructor(time, size, font, color, null, null, null, align, valign, null, null, fontOptions, new Rectangle(width, height, backgroundColor, borderColor, borderWidth).centerReg({add:false}));
        this.type = "Timer";
        var that = this;
        that.totalTime = time;
        ...
        this.start = function(t) {
            ...
            that.intervalID = setInterval(function () {
                ...
                else that.text = zim.decimals(that.time, 1, step%1000==0?0:1);
